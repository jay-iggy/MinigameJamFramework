    ...
    public class HummerController : MonoBehaviour {
        ...
        private void Update() {
            ...
            foreach (Collider col in Physics.OverlapBox(hitbox.transform.position, hitbox.size * 2)) {
                if (col.gameObject.GetComponent<IcebreakerPawn>() != null &&
                    Vector3.Distance(transform.position, col.gameObject.transform.position) < targetDist) {
                ...
                else if (col.gameObject.GetComponent<IcecubeController>() != null &&
                         Vector3.Distance(transform.position, col.transform.position) < targetDist) {
        ...
        public void SmashIce() {
            if (currentPenguin != null && !currentPenguin.GetStunned()) {
                ...
                currentIce.Smash();
                Instantiate(iceEffect, currentIce.transform.position + new Vector3(0f, 0.25f, 0f),
                    iceEffect.transform.rotation);
            }
        }
