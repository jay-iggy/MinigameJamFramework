...
namespace Icebreaker {
    public class IcebreakerPawnManager : MonoBehaviour {
        ...
        private void BindPawn(int playerIndex) {
            if (AreAllPawnsBound()) {
                Debug.LogWarning(
                    $"PawnManager: All pawns are already bound. Player index {playerIndex} was not bound.");
                return;
            }
        ...
        private void OnDisable() {
            PlayerJoinManager.onAllPlayersJoined
                .RemoveListener(BindPawns); // stop listening when this component is disabled
        }
