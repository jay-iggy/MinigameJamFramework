    ...
    public class IcecubeManager : MonoBehaviour {
        ...
        void Start() {
            ...
            for (int i = 0; i < width * height; i++) {
                ...
                Vector3 newPos = new Vector3(xOffset + column * template.transform.localScale.x,
                    template.transform.position.y, zOffset + row * template.transform.localScale.z);
                ...
                if (column > 0) {
                    clone.GetComponent<IcecubeController>().neighborLeft =
                        grid[i - 1].GetComponent<IcecubeController>();
                    grid[i - 1].GetComponent<IcecubeController>().neighborRight =
                        clone.GetComponent<IcecubeController>();
                }

                if (row > 0) {
                    clone.GetComponent<IcecubeController>().neighborDown =
                        grid[i - width].GetComponent<IcecubeController>();
                    grid[i - width].GetComponent<IcecubeController>().neighborUp =
                        clone.GetComponent<IcecubeController>();
                }
            }
        }
        ...
        void Update() {
            ...
            if (lastMelted > 2) {
                ...
                if (timer > suddenDeathTime) {
                    StartCoroutine(grid[random.Next(0, width * height - 1)].GetComponent<IcecubeController>()
                        .Flash(1, 3));
                    ...
                    for (int i = 0; i < height; i++) {
                        StartCoroutine(
                            grid[i * width + column].GetComponent<IcecubeController>().StartFreeze(false, 2f));
                    }
                }
                else {
                    StartCoroutine(grid[random.Next(0, width * height - 1)].GetComponent<IcecubeController>()
