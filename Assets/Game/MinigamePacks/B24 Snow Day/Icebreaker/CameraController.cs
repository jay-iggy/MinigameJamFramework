...
public class CameraController : MonoBehaviour
{
    ...
    IEnumerator Startup() {
        ...
        while (timer < 4) {
            ...
            transform.position += Time.deltaTime * 3 * (new Vector3(playerToZoomTo.transform.position.x, 6, playerToZoomTo.transform.position.z - 4) - transform.position);
    ...
    public IEnumerator EndSequence(GameObject lastDead) {
        ...
        yield return null;
        while(true) {
            transform.position += 6 * Time.deltaTime * (new Vector3(winner.transform.position.x, 7, winner.transform.position.z - 4) - transform.position);
            yield return null;
        }
    }
}
