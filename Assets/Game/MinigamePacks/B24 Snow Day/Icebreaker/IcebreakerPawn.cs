...
public class IcebreakerPawn : Pawn {
    ...
    private void Update() {
        ...
        _rigidbody.velocity = (gravity * Time.deltaTime * Vector3.up) + (_rigidbody.velocity + new Vector3(_moveInput.x * speed * (stunned ? 0.3f : 1), 0, _moveInput.y * speed * (stunned ? 0.3f : 1))) * Mathf.Pow(friction, Time.deltaTime + 1);
    ...
    protected override void OnActionPressed(InputAction.CallbackContext context) {
        ...
        // Swing Hummer
        if (!stunned && (context.action.name == "ButtonA" || context.action.name == "ButtonB") && Time.time > hammerTime + hammerCooldown)
        {
            Swing();
